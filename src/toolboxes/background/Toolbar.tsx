...
const Submenu = (props: SubmenuProps): ReactElement => {
  ...
  const handleClickAway = () => {
    // For some reason the clickaway handler runs when the user clicks to open the submenu,
    // but only on the first click - subsequent clicks on the toolbar button don't trigger this handler.
    // So we prevent that first spurious call from closing the submenu as it opens by aborting
    // the first time this handler runs.
    if (props.isChannelPinned || firstClick) {
      setFirstClick(false);
      return;
    }
    setButtonClicked("");
    props.handleOpen()(null); // close the submenu
  };
