 ...
 * @type {ViewPoint} 
 */
 const NULL_VIEW_POINT: ViewPoint = Object.freeze({
   ...
   * @type {CanvasBounds}
   */
  const NULL_BOUNDS: CanvasBounds = Object.freeze({
  ...
  export default class CoordinateSystem {
     ...
     */
    constructor({ scaleExtents: Extents, documentSize: Size }) {
     ...
     */
    set scaleExtents({ min, max }: Extents): void {
     ...
     */
    set documentSize({ width, height }: Size): void {
     ...
     */
    get canvasBounds(): CanvasBounds | undefined {
      if (this._canvas) {
     ...
     */
    get canvasRect(): {left: number, top: number} | undefined {
      if (this.canvas) {
     ...
     * assumed.
     */
    clientPointToViewPoint = ({ clientX, clientY }: ClientPoint, view: View = this._view): ViewPoint => {
     ...
     */
      ...
      * assumed.
      */
    viewPointToClientPoint = ({ x, y }: ViewPoint, view: View = this._view): ClientPoint => {
      const { left, top } = this.canvasRect || NULL_BOUNDS;
